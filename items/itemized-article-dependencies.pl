	...
	# This is (or ought to be) the case of constructor properties
	  ...
	  if ($item =~ / \/ ([a-z]+) \z/x) {
	    ...
	    if (scalar @generated_properties == 0) {
	      ...
	      croak ('Error: it appears that fragment ', $item_fragment, '  generates multiple constructor property that are congruent with the one we are searching for (', $item, ').  Which one should we choose?', "\n");
	    } else {
	    ...
	    if (scalar @generated_non_properties == 0) {
	      ...
	      croak ('Error: the non-property item ', $item, ' is congruent with multiple items generated by fragment ', $item_fragment, '; which one should we choose?', "\n");
	    } else {
