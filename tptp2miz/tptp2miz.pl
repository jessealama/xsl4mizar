...
unless (-e $tptp_file) {
  print 'Error: the supplied TPTP file,', "\n", "\n", '  ', $tptp_file, "\n", "\n", 'does not exist.', "\n";
  exit 1;
}
...
if ($which_tptp4X_exit_code != 0) {
  print 'Error: the tptp4X program is required, but does not appear to be available.', "\n";
  exit 1;
}
...
foreach my $stylesheet (@stylesheets) {
  ...
  unless (-e $stylesheet_path) {
    print 'Error: the required stylsheet ', $stylesheet, ' could not be found in the directory', "\n", "\n", '  ', $stylesheet_home, "\n", "\n", 'where we expect to find it.', "\n";
    exit 1;
  }
  unless (-r $stylesheet_path) {
    print 'Error: the required stylsheet ', $stylesheet, ' under', "\n", "\n", '  ', $stylesheet_home, "\n", "\n", 'is not readable.', "\n";
    exit 1;
  }
}
...
if ($tptp4X_xmlize_exit_code != 0) {
  print 'Error: tptp4X did not exit cleanly when XMLizing the TPTP file at', "\n", "\n", '  ', $tptp_file, "\n", "\n", 'Its exit code was ', $tptp4X_xmlize_exit_code, '.', "\n";
  exit 1;
}
...
unless (-e $tptp_xml) {
  print 'Error: tptp4X did not generate an XML version of the TPTP file at ', "\n", "\n", $tptp_file, "\n";
  exit 1;
}

unless (-r $tptp_xml) {
  print 'Error: the XML reformatting of', "\n", "\n", $tptp_file, "\n", "\n", 'generated by tptp4X is not readable.', "\n";
  exit 1;
}
...
if ($xmllint_exit_code != 0) {
  print 'Error: tptp4X failed to generate a valid XML document corresponding to the TPTP file at', "\n", "\n", '  ', $tptp_file, "\n";
  exit 1;
}
...
if ($tptp2voc_xsltproc_exit_code != 0) {
  print 'Error: xsltproc did not exit cleanly when making the vocabulary (.voc) file for', "\n", "\n", '  ', $tptp_file, "\n";
  exit 1;
}
...
foreach my $extension ('dno', 'dco') {
  ...
  unless (-e $stylesheet) {
    print 'Error: the required stylesheet for generating the .', $extension, ' file does not exist at the expected location (', $stylesheet, ').', "\n";
    exit 1;
  }
  ...
  if ($xsltproc_exit_code != 0) {
    print 'Error: xsltproc did not exit cleanly when generating the .', $extension, ' file for', "\n", "\n", '  ', $tptp_file, "\n", "\n", 'The exit code was ', $xsltproc_exit_code, "\n";
    exit 1;
  }
}
